    ...
    if optGit options
      then withSystemTempDirectory "hydra-clone" $ \tempDir -> do
        when (optVerbose options) $
          putStrLn $
            "Cloning repository: " ++ optPath options
        ...
        if Map.null results
          then do
            ...
            exitFailure
          else case optOutputFormat options of
            Console -> prettyPrint results
            fmt -> TIO.putStrLn $ formatOutput results fmt
        ...
        when (optVerbose options) $
          putStrLn $
            "Analyzing path: " ++ optPath options
